...
class _InvestmentsScreenState extends State<InvestmentsScreen> {
  ...
  final InvestmentService _investmentService = InvestmentService(); // Fallback
  ...
  Future<void> _loadInvestmentsPaginated({bool isRefresh = false}) async {
    ...
    try {
      ...
      // Użyj Firebase Functions do pobrania inwestycji
      ...
      if (mounted) {
        setState(() {
          if (isRefresh) {
            ...
            _filteredInvestments = result.investments;
            _displayedInvestments = result.investments;
            _currentPage = 1;
          } else {
            _investments.addAll(result.investments);
            _filteredInvestments.addAll(result.investments);
            _displayedInvestments.addAll(result.investments);
            _currentPage++;
          }
