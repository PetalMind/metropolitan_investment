...
class EnhancedVotingStatusService extends BaseService {
  ...
  Future<VotingStatusChangeResult> updateVotingStatusWithHistory(
    ...
    try {
      ...
      // Update cache
      _updateClientCache(clientId, currentClient.copyWith(votingStatus: newStatus));
      
      // Clear analytics cache for consistency
      _clearRelatedCaches();

      final duration = DateTime.now().difference(startTime);
      print('✅ [EnhancedVoting] Status updated in ${duration.inMilliseconds}ms for ${currentClient.name}');
  ...
  Future<VotingStatusChangeResult> _performVotingStatusTransaction({
    ...
    return await FirebaseFirestore.instance.runTransaction<VotingStatusChangeResult>(
      (transaction) async {
        ...
        if (!clientDoc.exists) {
          throw VotingStatusException('Klient został usunięty podczas aktualizacji');
        }
  ...
  Future<BatchVotingStatusResult> updateMultipleVotingStatuses(
    ...
    const chunkSize = 10; // Firestore transaction limit is 500 operations, but we're being conservative
  ...
  Future<List<VotingStatusChangeResult>> _processBatchChunk(
    ...
    return await FirebaseFirestore.instance.runTransaction<List<VotingStatusChangeResult>>(
      (transaction) async {
        final results = <VotingStatusChangeResult>[];
        final user = FirebaseAuth.instance.currentUser;
        ...
        for (final update in chunk) {
          try {
            ...
            if (previousStatus == update.newStatus) {
              results.add(VotingStatusChangeResult.noChange(
                clientId: update.clientId,
  ...
  Future<List<VotingStatusChange>> getRecentChanges({
    ...
    try {
      ...
      if (changeTypes != null && changeTypes.isNotEmpty) {
        query = query.where('changeType', whereIn: changeTypes.map((e) => e.name).toList());
      }
  ...
  Future<VotingStatusStatistics> getStatistics({
    ...
    final cacheKey = 'voting_stats_${fromDate?.millisecondsSinceEpoch}_${toDate?.millisecondsSinceEpoch}';
    
    if (useCache) {
      return await getCachedData<VotingStatusStatistics>(
        cacheKey,
        () async {
  ...
  Future<VotingStatusStatistics> _fetchStatistics(DateTime? fromDate, DateTime? toDate) async {
    try {
...
class VotingStatusStatistics {
  ...
  factory VotingStatusStatistics.fromChanges(
    ...
    for (final change in changes) {
      ...
      // Count by user
      changesByUser[change.editedBy] = (changesByUser[change.editedBy] ?? 0) + 1;

      // Count by new status
      if (change.newVotingStatus != null) {
        changesByStatus[change.newVotingStatus!] = (changesByStatus[change.newVotingStatus!] ?? 0) + 1;
      }

      // Count by day
      final dayKey = '${change.changedAt.year}-${change.changedAt.month.toString().padLeft(2, '0')}-${change.changedAt.day.toString().padLeft(2, '0')}';
